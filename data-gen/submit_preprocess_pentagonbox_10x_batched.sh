...
JDL=${SAILIR_DIR}/data-gen/preprocess_pentagonbox_10x_batch.jdl
...
mkdir -p ${SAILIR_DIR}/data-gen/logs
...
while (( OFFSET < TOTAL_SHARDS )); do
    ...
    cat > "${JDL}" <<EOF
universe                 = vanilla
executable               = ${SAILIR_DIR}/data-gen/preprocess_shard_pentagonbox_10x.sh
arguments                = \$(WorkerID)
...
output                   = ${SAILIR_DIR}/data-gen/logs/preprocess_pb_10x_shard_\$(WorkerID).out
error                    = ${SAILIR_DIR}/data-gen/logs/preprocess_pb_10x_shard_\$(WorkerID).err
log                      = ${SAILIR_DIR}/data-gen/logs/preprocess_pb_10x_shard_\$(WorkerID).log
