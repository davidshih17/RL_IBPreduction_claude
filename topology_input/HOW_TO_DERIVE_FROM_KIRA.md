...
> `--use-symmetry`): derive the COMPLETE family basis with the
> `master-integrals/` component (per-sector maximal-cut sweep) — see
> `master-integrals/README.md` for the full recipe and validation protocol.**
> ANY Kira/FIRE run with a target list yields a TARGET-SCOPED basis: the
> masters of the selected system, not of the family. SAILIR's cascades
> lawfully wander into every sector of the cone, so a target-scoped basis
> leaves workers grinding forever on integrals whose master slot is missing
> (this exact failure cost days on gravity3L, 2026-07: the FIRE-68 table
> basis lacked the sector-635 masters; the Kira 64-master run was equally
> target-scoped). A full-cone Kira run is prohibitive at 3 loops (73+ GB).
> The cut sweep gives the complete basis cheaply (gravity3L: 142 masters,
> ~5 min/sector on Condor) and is validated per-family (depth stability,
> FIRE/Kira spot checks, top-sector Kira cross-check). The installed basis
> is relabeled into SAILIR's canonical sectors by
> `reduction/canonical_masters.py` at run time (its gate must print ALL PASS).
