#!/bin/bash
# Probe (7,4) with FULL memory breakdown instrumentation.
# Uses current working tree (iraws+LRU). pympler.asizeof + tracemalloc on
# every persistent structure every 10 steps.
set -e
BASE=/het/p4/dshih/jet_images-deep_learning/SAILIR_phase2
OUTDIR=$BASE/results/probe_74_v6_memprobe
mkdir -p $OUTDIR
...
environment = "PYTHONUNBUFFERED=1 MALLOC_MMAP_THRESHOLD_=67108864 SAILIR_END_OF_STEP_TRIM=1 SAILIR_MEMPROBE_FULL=$OUTDIR/memprobe.jsonl SAILIR_MEMPROBE_FULL_STEPS=30,50,70,80"
