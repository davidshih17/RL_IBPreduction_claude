#!/bin/bash
# Thread-viability probe: split attach_aux (P4's 123s) into nogil-mergeable
# substitute_one vs GIL-bound glue. Decides whether threads (no IPC, no remap)
# can parallelize P4 (subone-dominated) or not (glue-dominated).
set -e
BASE=/het/p4/dshih/jet_images-deep_learning/SAILIR_phase2
OUTDIR=$BASE/results/probe_74_v7_tprobe; mkdir -p $OUTDIR
...
environment = "PYTHONUNBUFFERED=1 MALLOC_MMAP_THRESHOLD_=67108864 SAILIR_END_OF_STEP_TRIM=1 SAILIR_TABU_CAP=0 SAILIR_STRIP_RAWS=1 SAILIR_PACKED_RS=1 SAILIR_P4_TPROBE=1"
