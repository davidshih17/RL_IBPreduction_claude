#!/usr/bin/env python
"""v5 beam search: strip weight-passenger content from expr AND resolved_subs.
Drop `subs` from State entirely; feed dummy subs (RS keys + empty repls)
to the model. Final full-expression recovery is via path replay.

DESIGN — see DESIGN_v5_iraws.md follow-up section.

State:
    expr           target-sector dict, STRIPPED of integrals with weight (w1,w2)
                   strictly below the STARTING integral's (w1,w2)
    resolved_subs  same keys as historical subs; each value dict STRIPPED of
                   passenger integrals (weight below start)
    score          cumulative log(action_prob)
    path           list of (target, ibp_op, delta) tuples for replay
    n_non_masters  count in expr (post-strip; only active-bucket non-masters)
    sub_accum      sub-sector passenger dict (Option F behavior, unchanged)

NOT in State (vs prior beam_search_full):
    subs           dropped — model gets RS keys as dummy
    aux_flat       dropped — iraws recomputed fresh per step
    indirect_aux   dropped

Single-process, no checkpointing, no worker pool — minimal to validate
that the (8,4) integral still reduces under these strippings.
"""
import argparse
import math
import os
...
def beam_search_v5(env, model, start_expr, target_sector, start_w12,
    ...
    if resume_from is not None and os.path.exists(resume_from):
        ...
        if verbose:
            print(f'[v5 RESUME] loaded {len(beam)} beam states from step '
    ...
    if (resume_from is not None and os.path.exists(resume_from)
        ...
        if verbose:
            ...
            print(f'[v5 RESUME] restored tabu_dict: {len(tabu_dict)} expr '
    ...
    for step in range(resume_step, max_steps):
        ...
        if winning is not None:
            best_state = winning
            if verbose:
                print(f'[v5 step {step}] beam state drained — DONE '
        ...
        if not tasks:
            if verbose:
                print(f'[v5 step {step}] no tasks — STUCK', flush=True)
            break
        ...
        if not candidates:
            if verbose:
                print(f'[v5 step {step}] no successful candidates — STUCK', flush=True)
            break
        ...
        if verbose:
            ...
            print(f'[v5 step {step+1:>3}] beam={len(beam)} '
