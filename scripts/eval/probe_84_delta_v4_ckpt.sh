#!/bin/bash
# Clean v4 with default thin checkpointing. Same env as v4, but the worker
# now writes a thin checkpoint every 50 steps to $OUTDIR/result.pkl.ckpt.r1
# (path auto-derived from --output). After step ~150 we resume with
# DELTA_DEBUG_COMPARE=1 to diff iraws against from-scratch baseline.
set -e
BASE=/het/p4/dshih/jet_images-deep_learning/SAILIR_phase2
OUTDIR=$BASE/results/probe_84_delta_v4_ckpt
mkdir -p $OUTDIR
