#!/bin/bash
# v4+rescue from scratch with DELTA_DEBUG_COMPARE=1. Compare fires on EVERY
# beam survivor at every step. If iraws is genuinely action-equivalent to
# baseline, NO fires through the run. If not, the first fire pinpoints the
# step+state where v4-rescue diverges.
set -e
BASE=/het/p4/dshih/jet_images-deep_learning/SAILIR_phase2
OUTDIR=$BASE/results/probe_84_delta_v4rescue_dbgcompare_scratch
mkdir -p $OUTDIR
