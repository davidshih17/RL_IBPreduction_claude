...
OUTDIR=$BASE/results/pentagonbox_8_5_v6_round2/memray_trace
rm -rf $OUTDIR; mkdir -p $OUTDIR
PY=/het/p4/dshih/jet_images-deep_learning/RL_MIR_IBP/conda_env/bin/python
MODEL=$BASE/checkpoints/pentagonbox_10x_loop_100/best_model.pt
TOPOLOGY=$BASE/topology_input/pentagonbox
WORKER=$BASE/scripts/eval/onestep_worker_v6.py
# Wrapper: run worker under memray (bounded steps), then emit text reports.
...
$PY -m memray run --aggregate --trace-python-allocators --force \
    ...
    --beam_width 40 --max_steps 40 --prime 1009 --device cpu -v \
    --no-paper-masters-only > $OUTDIR/worker.log 2>&1
echo "=== memray summary (peak allocations by source location) ===" > $OUTDIR/report.txt
$PY -m memray summary $OUTDIR/capture.bin >> $OUTDIR/report.txt 2>&1 || true
$PY -m memray flamegraph --force -o $OUTDIR/flame.html $OUTDIR/capture.bin >> $OUTDIR/report.txt 2>&1 || true
$PY -m memray table --force -o $OUTDIR/table.html $OUTDIR/capture.bin >> $OUTDIR/report.txt 2>&1 || true
