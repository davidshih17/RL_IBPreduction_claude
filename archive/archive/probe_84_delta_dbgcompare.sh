#!/bin/bash
# v4 + DELTA_DEBUG_COMPARE=1 → on EVERY survivor's _materialize_aux, also
# rebuild the from-scratch baseline iraws and diff (op, seed) sets. Prints
# the first action-emitting missed entry with anchor sub_int and culprit_K.
# One-shot: stops after first divergence. CPU is ~2x baseline due to the
# parallel rebuild — fine for diagnostic runs only.
set -e
BASE=/het/p4/dshih/jet_images-deep_learning/SAILIR_phase2
OUTDIR=$BASE/results/probe_84_delta_dbgcompare
mkdir -p $OUTDIR
